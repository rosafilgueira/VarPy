...
def exp_rate(t, k, lam):
    rate = []
    rate = k*np.exp(lam*t)
    return rate

#2. Total functin for exponential model:
def exp_total(t, t0, k, lam):
    total = []
...
def creep_rate(t, k1, c, p1, k2, tf, p2):
    rate = []
    rate = k1*(t+c)**-p1 + k2*(tf-t)**-p2
    return rate

#7. Total function for Creep model, t1 = 0.0
def creep_total(t, t0, k1, c, p1, k2, tf, p2):
    total = []
